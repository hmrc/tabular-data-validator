...
import uk.gov.hmrc.services.validation.{ValidationError, DataValidator, Cell}
...
class StressTest extends WordSpec with Matchers {
  ...
  "validate the file" in {
    ...
    val inputFileName = "/data/CSV_10000rows_header_nf.csv"

    val (header :: testData) = readInpuCsv(inputFileName)

    val config = loadConfig("/config/validation-fix.conf").getConfig("validation-config")

    DataValidator(config).validateRows(testData, Some(new ValidationContext), firstRowNum = 2, ignoreBlankRows = false) match {
      case Some(errors) => {
        errors.collect  {
          case error @ ValidationError(Cell("A",_,_), ruleId: String, errorId: String, errorMsg: String)  => {
          }
        }
        errors.length should equal(29080)
      }
